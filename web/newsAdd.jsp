...
<%-- Content --%>
<div class="block1">
    <form action="ServletNews" method="post">
            ...
            <c:choose>
                <c:when test = "${param['action'] == 'update'}">
                    <sql:query var="result" dataSource="jdbc/db">
                        SELECT name,description,author FROM news where news_id = ${param['news_id']}
                    </sql:query>
                    <c:forEach items="${result.rows}" var="row">
                        <input type="text" name="news_id" value="${param['news_id']}" hidden>
                        <tr>
                            <td>Name:</td>
