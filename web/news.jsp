    ...
    <table>
        ...
        <c:forEach items="${result.rows}" var="row">
            <tr id="tr${row.news_id}">
                <td>${row.name}</td>
                <td>${row.author}</td>
                <td id="td_update ${row.news_id}"><a class="btnLink" href="newsAdd.jsp?action=update&news_id=${row.news_id}&author=${row2.id}" onmouseover="updrecolor(${row.news_id})" onmouseleave="upddecolor(${row.news_id})">UPDATE</a></td>
